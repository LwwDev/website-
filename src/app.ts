...
let currentSection: SectionId = 'work';

function scrollToSection(sectionId: SectionId, pushHistory = true): void {
...
function setupHashNavigation(): void {
    if (window.location.hash) {
        const sectionId = window.location.hash.replace('#', '') as SectionId;
...
function setupCursorFx(): void {
    ...
    function syncCursorClasses(): void {
    ...
    function spawnSparks(x: number, y: number, burst = 1): void {
    ...
    function updatePointer(x: number, y: number, target: EventTarget | null): void {
        targetX = x;
        targetY = y;
        setVisible(true);
...
async function setupDynamicIntro(): Promise<void> {
    ...
    try {
        ...
        intro.replaceChildren(
            ...
            ipSpan,
            document.createTextNode(". You made it to my corner of the internet. This is where I keep the projects, experiments, and bits of life that have stuck with me."),
        );
    } catch {
        setFallback();
    }
...
function setupTravelMap(): void {
    ...
    function isVisitedCountry(feature: { id?: string; properties?: { name?: string } }): boolean {
    ...
    async function renderMap(): Promise<void> {
        ...
        try {
            ...
            const countries = topojson.feature(world, world.objects.countries).features as Array<{ id?: string; properties?: { name?: string } }>;
            ...
            countries.forEach(feature => {
                ...
                countryPath.setAttribute('d', path(feature as object) || '');
...
function setupActivityChart(): void {
...
function setupProjectArchiveTabs(): void {
    const tabButtons = Array.from(document.querySelectorAll<HTMLButtonElement>('.project-tab-btn'));
    const panels = Array.from(document.querySelectorAll<HTMLElement>('.project-panel'));

    if (!tabButtons.length || !panels.length) return;

    function activateTab(panelName: string, focusTab = false): void {
    ...
    tabButtons.forEach((button, index) => {
        button.addEventListener('click', () => {
            ...
            if (panelName) activateTab(panelName);
        });
        ...
        button.addEventListener('keydown', event => {
            ...
            if (nextPanel) activateTab(nextPanel, true);
        });
    });
}
...
function setupKanban(): void {
    ...
    const toolbarMetaEl: HTMLElement = toolbarMeta;
    ...
    const dialogModeEl: HTMLElement = dialogMode;
    const dialogTitleEl: HTMLElement = dialogTitle;
    const closeButtonEl: HTMLButtonElement = closeButton;
    const cancelButtonEl: HTMLButtonElement = cancelButton;
    const deleteButtonEl: HTMLButtonElement = deleteButton;
    const saveButtonEl: HTMLButtonElement = saveButton;
    const summaryInputEl: HTMLInputElement = summaryInput;
    const descriptionInputEl: HTMLTextAreaElement = descriptionInput;
    const typeInputEl: HTMLSelectElement = typeInput;
    const priorityInputEl: HTMLSelectElement = priorityInput;
    const statusInputEl: HTMLSelectElement = statusInput;
    const assigneeInputEl: HTMLInputElement = assigneeInput;
    const labelsInputEl: HTMLInputElement = labelsInput;
    ...
    function loadKanbanState(): KanbanBoardState {
        ...
        try {
            ...
            if (!parsed || !Array.isArray(parsed.issues)) return fallback;

            const issues = parsed.issues
                .filter((issue): issue is KanbanIssue => {
                    if (!issue || typeof issue !== 'object') return false;
                    const candidate = issue as Partial<KanbanIssue>;
            ...
            if (rawOrder && typeof rawOrder === 'object') {
                kanbanColumnIds.forEach(columnId => {
                    const orderList = (rawOrder as Partial<Record<KanbanColumnId, unknown>>)[columnId];
...
function setupGraph(): void {
    ...
    const raw: Array<Pick<GNode, 'id' | 'label' | 'type' | 'r' | 'sectionId' | 'description'>> = [
    ...
    function setNodeTarget(id: string, x: number, y: number): void {
    ...
    function updateTargets(): void {
        ...
        const detailLayout: Array<{ id: string; parent: SectionId; angleOffset: number; distanceBias: number }> = [
    ...
    function connectedToHovered(n: GNode): boolean {
        const hovered = hoveredNode as GNode | null;
    ...
    function edgeTouchesSection(edge: GEdge, sectionId: SectionId): boolean {
    ...
    function nodeFill(n: GNode, isHovered: boolean, isConnected: boolean, isActive: boolean): string {
        if (n.type === 'core') return '#EEEEEE';
        if (isHovered) return '#F6F6F6';
        if (isActive) return '#CFCFCF';
        if (isConnected) return '#9A9A9A';
        return n.type === 'menu' ? '#5C5C5C' : '#2E2E2E';
    }

    function graphStatusText(node: GNode | null): string {
        if (!node) {
        ...
        if (node.sectionId) parts.push(`click to jump to ${sectionLabel(node.sectionId)}`);
        return parts.join(' · ');
    }
    ...
    function render(): void {
        ...
        edges.forEach(e => {
            ...
            const lit = hovered ? (e.source === (hovered as GNode).id || e.target === (hovered as GNode).id) : false;
    ...
    canvas.addEventListener('click', () => {
        const hovered: GNode | null = hoveredNode;
    ...
    const observer = new IntersectionObserver((entries) => {
        ...
        if (!visible) return;
        const sectionId = visible.target.id as SectionId;
        if (sectionIds.includes(sectionId)) {
            currentSection = sectionId;
    ...
    function loop(): void { tick(); render(); requestAnimationFrame(loop); }
    loop();
}
...
function setupNotes(): void {
    ...
    if (!(listEl instanceof HTMLElement) ||
        ...
        !(doneBtn instanceof HTMLButtonElement)) return;

    const list: HTMLElement = listEl;
    const dialog: HTMLDialogElement = dialogEl;
    const dateSpan: HTMLElement = dateEl;
    const tagSpan: HTMLElement = tagEl;
    const bodyArea: HTMLTextAreaElement = bodyEl;
    const done: HTMLButtonElement = doneBtn;

    let notes: NoteEntry[] = ((): NoteEntry[] => {
        try {
    ...
    dialog.addEventListener('click', e => { if (e.target === dialog) closeNote(); });
    dialog.addEventListener('cancel', e => { e.preventDefault(); closeNote(); });

    renderNotes();
}

function setupSnakeGame(): void {
    ...
    if (!(canvas instanceof HTMLCanvasElement) ||
        ...
        !(statusEl instanceof HTMLElement)) return;

    // Typed aliases so closures don't lose narrowing
    const cv: HTMLCanvasElement = canvas;
    const se: HTMLElement = scoreEl;
    const be: HTMLElement = bestEl;
    const sb: HTMLButtonElement = startBtn;
    const st: HTMLElement = statusEl;
...
function setupScrollProgress(): void {
    ...
    // build bars
    const bars: Map<SectionId, HTMLElement> = new Map();
